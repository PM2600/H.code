...
public class CartServiceImpl implements ICartService {
    ...
    public Integer addNum(Integer cid, Integer uid, String username) {
        ...
        Product product = productService.findById(result.getPid());
        if(result.getNum() >= product.getNum()){
            return -1;
        }
    ...
    public Integer subNum(Integer cid, Integer uid, String username) {
        ...
        if(result.getNum() <= 1){
            Integer[] cids = {cid};
            cartMapper.deleteVOByCids(cids);
            return 0;
        }
        // 可选：检查商品的数量是否大于多少(适用于增加数量)或小于多少(适用于减少数量)
        Integer num = result.getNum() - 1;
    ...
    @Override
    public void deleteVOByCids(Integer[] cids) {
//        List<CartVO> list = cartMapper.findVOByCids(cids);
//        Iterator<CartVO> it = list.iterator();
//        while(it.hasNext()){
//            CartVO cart = it.next();
